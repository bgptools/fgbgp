...
func DecodeAttributes2B(buf io.Reader, attrlen uint16) ([]messages.BGPAttributeIf, error) {
	b := make([]byte, attrlen)
	binary.Read(buf, binary.BigEndian, b)
...
var stupidv6MaskHack = [][]byte{
	...
	{0xff, 0xff, 0xff, 0xff, 0xfc, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
	...
	{0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xf0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
	...
	{0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
	...
	{0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xe0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
	...
	{0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfc, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
...
func DecodeBGP4TD1(buf io.Reader, timestamp time.Time, subtype uint16, length uint32) (Mrt, error) {
	out := MrtTableDumpV1_Rib{}
	out.Timestamp = timestamp
	binary.Read(buf, binary.BigEndian, &out.ViewNumber)
	binary.Read(buf, binary.BigEndian, &out.SequenceNumber)
	var rawPrefix []byte
	var mask []byte
	var prefixLen uint8
	switch subtype {
	case SUBT_TABLE_DUMP_AFI_IPV4:
		...
		_, err := io.ReadFull(buf, rawPrefix)
		if err != nil {
			return out, err
		}
	case SUBT_TABLE_DUMP_AFI_IPV6:
		rawPrefix = make([]byte, 16)
		_, err := io.ReadFull(buf, rawPrefix)
		if err != nil {
			return out, err
		}
	...
	var originUnixTime uint32
	binary.Read(buf, binary.BigEndian, &originUnixTime)
	...
	out.PeerIP = net.IP(peerIP)
	binary.Read(buf, binary.BigEndian, &out.PeerAS)
	var attrlen uint16
	binary.Read(buf, binary.BigEndian, &attrlen)
	attrs, err := DecodeAttributes2B(buf, attrlen)
	if err != nil {
		return out, err
	}

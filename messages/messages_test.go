...
func TestNilParameterString(t *testing.T) {
	blob := []byte{0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x03, 0x30, 0x00, 0x30}
	msg, err := ParseOpen(blob)
	if err != nil {
		return
	}
	_ = msg.String()
...
func TestWriteAttributeHeaderRoundTrip(t *testing.T) {
	blob := []byte{0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00, 0x3a, 0x02, 0x00, 0x00, 0x00, 0x1f, 0x30, 0x30, 0x00, 0x00, 0x41, 0x30, 0x0a, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x41, 0x30, 0x04, 0x30, 0x30, 0x30, 0x30, 0x41, 0x30, 0x04, 0x30, 0x30, 0x30, 0x30, 0x16, 0x30, 0x30, 0x30}
	bgptype, bodylen, err := ParsePacketHeader(blob)
	if err != nil {
		return
	}
	...
	_, _, err = ParsePacketHeader(serialized)
	if err != nil {
		t.Fatal(err)
	}
}
